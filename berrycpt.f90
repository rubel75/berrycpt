...
USE generaloam_mod, ONLY: generaloam
...
COMPLEX(kind=dp), ALLOCATABLE :: &
    oamgnrlz(:,:,:), & ! generalized OAM(i,j)
    oamgnrlzdg(:,:) ! array to store generalized OAM(i,j) of a block of degenerate bands
LOGICAL :: &
...
DO ispin = 1, nstot
    ...
    END IF
    fnameout32 = 'oam_generalized_ij.dat'
    ...
    DO WHILE (.not.(fmommatend)) ! loop over k-points until the file ends
        ...
        ALLOCATE( oam(nvb,3) )
        ALLOCATE( oamgnrlz(nvb,nvb,3) )
        bcurv = 0.0_dp
        oam = 0.0_dp
        oamgnrlz = (0.0_dp, 0.0_dp)
        ...
        DO ig = 1, dg_group(nvb)
            ...
            DO ivoigt = 4, 6
                ...
                ! check returned arrays
                IF (.NOT. ALLOCATED(oamgnrlzdg)) ERROR STOP 'Array oamgnrlzdg is not allocated.'
                IF (ANY(SHAPE(oamgnrlzdg) /= [nvb,nvb])) THEN
                    WRITE (*,*) 'Array "oamgnrlzdg" has shape', SHAPE(oamgnrlzdg)
                    WRITE (*,*) 'while expected', [nvb,nvb]
                    ERROR STOP "Shape mismatch in array oamgnrlzdg"
                END IF
                ...
                DO m = 1, nmg
                    ...
                    oamgnrlz(members(m), idg1:nvb, ivoigt-3) = &
                        oamgnrlzdg(members(m), idg1:nvb)
                    oamgnrlz(idg1:nvb, members(m), ivoigt-3) = &
                        oamgnrlzdg(idg1:nvb, members(m))
                END DO
                DEALLOCATE(pijA, pijB, dEijdg, bcurvdg, oamdg, oamgnrlzdg)
        ...
        DO ivoigt = 4, 6
            WRITE(32,'(A,I0)') & !...
                '# Voigt index: ', ivoigt
            DO ivb = 1, nvb
                WRITE(32,wformat32) (oamgnrlz(ivb, j, ivoigt-3), j=1,nvb)
            END DO ! ivb
        END DO ! ivoigt
        DEALLOCATE( bcurv, oam, oamgnrlz )
