...
SUBROUTINE goamfn(nb, nvb, idg1, idg2, dg_group, & ! <- args in 
...
COMPLEX(kind=dp), ALLOCATABLE, intent(out) :: &
    goamdg(:,:)  ! Berry curvature for a block of degenerate bands 
...
ALLOCATE( M(nvb,nvb), Mcorr(nvb,nvb), goamdg(nvb,nvb) )
    ...
    DO j = i, nvb
        ...
        DO n = 1, nb
            ...
            IF ( dg_group(n) /=  dg_group(i) ) THEN ! bands 'i' and 'n' are _not_ in the same degenerate block, which also excludes n = i
                ...
                ! The prefactor (i*hbar/2*m0) becomes i/2 because we work in a.u. hbar = 1, m0 = 1
                Lnlm = ((0.0_dp, 1.0_dp)/2.0_dp) * CMPLX(p2, kind=dp)/REAL(dEij(i,n), kind=dp)
            ...
            END IF ! term 1
            ! Term 2:
            ! L2_{i,j} = (i*hbar/2*m0)*SUM_{n /= j, with i /in D} (px_{i,n}*py_{n,j} - py_{i,n}*px_{n,j})/(E_{j} - E_{n})
            ! here D is a subset of degenerate bands in the range [idg1, idg2]
            IF ( dg_group(n) /=  dg_group(j) ) THEN ! bands 'j' and 'n' are _not_ in the same degenerate block, which also excludes n = j
                ...
                ! The prefactor (i*hbar/2*m0) becomes i/2 because we work in a.u. hbar = 1, m0 = 1
                Lnlm = ((0.0_dp, 1.0_dp)/2.0_dp) * CMPLX(p2, kind=dp)/REAL(dEij(j,n), kind=dp)
                ! make sure Lnlm is finite (not NaN and not Inf)
                IF (.not. IEEE_IS_FINITE(AIMAG(Lnlm)) &
                    .or. .not. IEEE_IS_FINITE(REAL(Lnlm,dp))) THEN
                    WRITE(*,*) 'i =', i
...
DEALLOCATE( M, Mcorr )
RETURN
END SUBROUTINE goamfn

END MODULE goamfn_mod
